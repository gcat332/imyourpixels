...
test('the game page contains one in-scene car interaction and an editable check-in dialogue', async () => {
  const html = await readFile(new URL('../index.html', import.meta.url), 'utf8');
  for (const id of ['scene', 'car-button', 'driver', 'cue', 'dialog-box', 'dialog-copy', 'answer-form', 'answer-input', 'answer-options']) {
